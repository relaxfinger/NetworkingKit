...
NativeNetwork 是一个面向 iOS 与 macOS App 的轻量级原生 Swift 网络库，支持 REST、GraphQL、`async/await`、Combine、Swift 6 并发、Client 级配置、错误本地化和拦截器。
...
然后把 `NativeNetwork` 添加到使用它的 target。
...
打开 `Examples/NativeNetworkDemo/NativeNetworkDemo.xcodeproj`，运行 `NativeNetworkDemo-iOS` 或 `NativeNetworkDemo-macOS`。Demo 覆盖 REST、GraphQL、App 级配置、英语/简体中文错误本地化，以及内置和自定义拦截器。
