...
缓存遵守请求和响应中的 `Cache-Control: no-store`、`no-cache`，并支持 `Expires`；会为 `Vary` Header 保留独立响应变体，且永不缓存 `Vary: *` 响应。`304 Not Modified` 会先与旧缓存 Header 合并再刷新 TTL，内存缓存则采用最近最少使用（LRU）淘汰顺序。
